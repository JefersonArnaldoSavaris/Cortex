...
The current runner now persists analyses and timeline events in a relational database. For local development it defaults to `sqlite:///./tradingagents_app.db`, and for production it can point to PostgreSQL with `DATABASE_URL` or `TRADINGAGENTS_DATABASE_URL`.
